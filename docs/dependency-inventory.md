# Inventário de dependências Python

Este inventário foi gerado a partir do ambiente Python instalado no container `web`.
Ele complementa `THIRD_PARTY_NOTICES.md` e deve ser regenerado após atualizações de dependências.

| Pacote | Versão | Licença declarada |
| --- | ---: | --- |
| anyascii | 0.3.3 | OSI Approved :: ISC License (ISCL) |
| arabic-reshaper | 3.0.1 | MIT |
| asgiref | 3.11.1 | BSD-3-Clause |
| asn1crypto | 1.5.1 | MIT |
| beautifulsoup4 | 4.15.0 | MIT License |
| certifi | 2026.6.17 | MPL-2.0 |
| cffi | 2.0.0 | MIT |
| charset-normalizer | 3.4.7 | MIT |
| coloraide | 3.3.1 | MIT |
| cryptography | 48.0.1 | Apache-2.0 OR BSD-3-Clause |
| cssselect2 | 0.9.0 | OSI Approved :: BSD License |
| defusedxml | 0.7.1 | PSFL |
| Django | 5.2.15 | BSD-3-Clause |
| django-filter | 25.2 | OSI Approved :: BSD License |
| django-formtools | 2.6.1 | BSD |
| django-modelcluster | 6.5 | BSD-3-Clause |
| django-otp | 1.7.0 | Unlicense |
| django-permissionedforms | 0.1 | BSD |
| django-phonenumber-field | 8.4.0 | MIT |
| django-stubs-ext | 6.0.5 | MIT |
| django-taggit | 6.1.0 | BSD |
| django-tasks | 0.12.0 | BSD-3-Clause |
| django-treebeard | 5.2.2 | Apache-2.0 |
| django-two-factor-auth | 1.17.0 | MIT |
| djangorestframework | 3.17.1 | BSD-3-Clause |
| draftjs_exporter | 5.2.0 | MIT |
| et_xmlfile | 2.0.0 | MIT |
| filetype | 1.2.0 | MIT |
| gunicorn | 23.0.0 | MIT |
| html5lib | 1.1 | MIT License |
| idna | 3.18 | BSD-3-Clause |
| laces | 0.1.2 | OSI Approved :: BSD License |
| lxml | 6.1.1 | BSD-3-Clause |
| modelsearch | 1.3.1 | BSD-3-Clause |
| openpyxl | 3.1.5 | MIT |
| oscrypto | 1.3.0 | MIT |
| packaging | 26.2 | Apache-2.0 OR BSD-2-Clause |
| phonenumberslite | 9.0.32 | Apache-2.0 |
| pillow | 12.2.0 | MIT-CMU |
| pillow_heif | 1.4.0 | BSD-3-Clause |
| pip | 26.1.2 | MIT |
| psycopg | 3.2.13 | LGPL-3.0-only |
| psycopg-binary | 3.2.13 | LGPL-3.0-only |
| pycparser | 3.0 | BSD-3-Clause |
| pyHanko | 0.35.1 | MIT |
| pyhanko-certvalidator | 0.31.1 | MIT |
| pypdf | 6.13.3 | BSD-3-Clause |
| pypng | 0.20220715.0 | MIT |
| python-bidi | 0.6.10 | OSI Approved :: GNU Library or Lesser General Public License (LGPL) |
| PyYAML | 6.0.3 | MIT |
| qrcode | 7.4.2 | BSD |
| reportlab | 4.5.1 | BSD license (see license.txt for details), Copyright (c) 2000-2025, ReportLab Inc. |
| requests | 2.34.2 | Apache-2.0 |
| six | 1.17.0 | MIT |
| soupsieve | 2.8.4 | MIT |
| sqlparse | 0.5.5 | OSI Approved :: BSD License |
| svglib | 2.0.2 | LGPL-3.0-or-later |
| telepath | 0.3.1 | BSD |
| tinycss2 | 1.5.1 | OSI Approved :: BSD License |
| typing_extensions | 4.15.0 | PSF-2.0 |
| tzlocal | 5.4.3 | MIT |
| uritools | 6.1.2 | MIT |
| urllib3 | 2.7.0 | MIT |
| wagtail | 7.4.2 | BSD-3-Clause |
| webencodings | 0.5.1 | BSD |
| whitenoise | 6.11.0 | MIT |
| Willow | 1.12.0 | OSI Approved :: BSD License |
| xhtml2pdf | 0.2.17 | Apache License |
